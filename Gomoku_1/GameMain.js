...
function onMouseClick(e)
{
    ...
    else
    {
        ...
        else if(userToRemote)//远程对战
        {
            if(yourTurn)//控制我是否可以下
            {
                if(isBlack)//控制我下的颜色
                {
                    playChess(1, row, col);
                    writeChess(row,col,1);
                    yourTurn=false;
                }
                else
                {
                     playChess(2, row, col);
                     writeChess(row,col,2);
                     yourTurn=false;
                }
            }
            else
...
{
    var count1=searchWestToEastSum(chessColor,row,col);//统计东西方向的落子数
    var count2=searchNorthToSouthSum(chessColor,row,col);//统计南北方向的落子数
    var count3=searchSouthWestToNorthEast(chessColor,row,col);//统计东北--西南的落子数
    var count4=searchNorthWestToSouthEast(chessColor,row,col);//统计西北--东南的落子数`
